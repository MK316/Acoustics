import streamlit as st

st.markdown("""
  #### 🐾 Phonology & English Education
  - Semester: Spring 2025
  - Topic: Acoustics and Auditory Phonetics
  """)

st.image(
    "https://github.com/MK316/Acoustics/raw/main/images/acoustics-logo4.png",
    width=600,  # Adjust width as needed
    caption= "Physical aspect of sound; What is sound, and how does it relate to teaching/learning spoken English?; How do English learners perceive sounds differently based on their linguistic background?"
)

st.markdown("---")
st.markdown("⛺Goto [MK316 home](https://mk316.github.io)")
st.caption("🕑 Last updated: Feb.11, 2025")
